...
class Clock:
    ...
    # 월별 일 수 정의 (윤년이 아닐 경우)
    @staticmethod
    def get_days_in_month(_year, _month):
        if _month == 2:
            # 윤년 조건
    ...
    def get_sleep_sec(self, sec):
        for i in range(sec) : # sec 초 sleep
            for _ in range(self.loop_per_second) :
                self.__tick()
        print(f'{sec}초 sleep')
...
class DummySensor:
    ...
    def set_env(self, current_time):
        ...
        with open(log_file, 'a') as log :
            values = list(map(lambda x: str(f'{x[1]:.2f}'), DummySensor.__env_values.values()))
            log.write(f'{current_time} | ' + ' | '.join(values) + '\n')

ds = DummySensor()     # DummySensor 객체 생성

one_second = 100000
clock = Clock(one_second) # Clock 객체 생성 / 1초 - 10000/loop

for _ in range(10) :
    # 환경 변수 값 설정(random)
    print(f'변수 값 설정(random)')
    ds.set_env(clock.get_time())
